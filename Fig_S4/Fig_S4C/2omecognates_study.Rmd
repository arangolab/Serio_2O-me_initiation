...
codon_counts_by_file <- filtered_plus1 %>%
  mutate(
    Study = str_extract(FileName, "^[^A-Z]+"),
    Cell_Line = gsub(".*?([A-Z][a-zA-Z0-9]*)Cognates.*", "\\1", FileName)
  ) %>%
  group_by(FileName, Study, Cell_Line, codon) %>%
  summarise(Count = n(), .groups = "drop") %>%
  arrange(FileName, desc(Count))

# get cell lines
unique_cell_lines <- unique(codon_counts_by_file$Cell_Line)

# plot Nm[+1] counts per codon, new plot per cell line
for (cell in unique_cell_lines) {
  cell_data <- codon_counts_by_file %>%
  ...
  p <- ggplot(cell_data, aes(x = codon, y = Count, fill = fill_color)) +
    geom_bar(stat = "identity", width = 0.7, color = NA) +
    scale_fill_identity() +
    labs(
      title = paste("Prevalence of Nm[+1] per Codon in", cell),
      x = "Codon",
      y = "Number of Sites"
    ) +
