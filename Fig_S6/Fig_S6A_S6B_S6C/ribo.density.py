...
# Read the BED file
try:
    bed_df = pd.read_csv(bed_file, sep="\t", header=None, names=["chrom", "start", "end", "codon", "strand"], dtype={"chrom": str, "start": int, "end": int, "codon": str, "strand": str})
except Exception as e:
...
for index, row in bed_df.iterrows():
    ...
    try:
        ...
        # Create a result row
        result_row = [chrom, start, end, row["codon"], strand] + list(rpkm_values)
...
results_df = pd.DataFrame(results, columns=["chrom", "start", "end", "codon", "strand"] + col_labels)
